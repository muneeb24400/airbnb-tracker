/**
 * App.jsx - Root component with Analytics tab added
 */
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-primary)" }}>
      ...
      <header style={{
        ...
        <div className="container" style={{ display: "flex", alignItems: "center", padding: "0 24px", height: 64, gap: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginRight: "auto" }}>
            <div style={{ width: 36, height: 36, borderRadius: "10px", background: "linear-gradient(135deg, var(--accent-gold), #b8922e)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🏠</div>
            <div>
              <h1 style={{ fontSize: 17, fontFamily: "var(--font-display)", fontWeight: 600, letterSpacing: "-0.01em" }}>StayTrack</h1>
              <p style={{ fontSize: 10, color: "var(--text-muted)", letterSpacing: "0.08em", textTransform: "uppercase", marginTop: -2 }}>Booking Manager</p>
            </div>
          </div>
            ...
            {TABS.map((tab) => (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)} className="btn" style={{
                ...
                {tab.key === "dashboard" && upcomingCount > 0 && (
                  <span style={{ marginLeft: 6, background: "var(--accent-rose)", color: "#fff", borderRadius: "99px", fontSize: 10, padding: "1px 6px", fontWeight: 700 }}>
                    {upcomingCount}
                  </span>
                )}
              </button>
          ...
          <button className="btn btn-ghost" onClick={loadBookings} disabled={fetchLoading} style={{ padding: "7px 12px", fontSize: 13 }} title="Refresh">
        ...
        {activeTab === "dashboard" && (
          <div>
            ...
            {fetchLoading
              ...
              : <BookingsList bookings={bookings} onDelete={handleDeleteBooking} deleteLoading={deleteLoading} />}
          </div>
        )}
        ...
        {activeTab === "add" && (
          <div style={{ maxWidth: 860, margin: "0 auto" }}>
            <div style={{ marginBottom: 24 }}>
              ...
              <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>This will be saved directly to your Google Sheet.</p>
            </div>
      ...
      <footer style={{ padding: "20px 24px", textAlign: "center", color: "var(--text-muted)", fontSize: 12, borderTop: "1px solid var(--border)", marginTop: 40 }}>
